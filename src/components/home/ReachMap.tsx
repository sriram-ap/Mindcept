...
export function ReachMap({ locations }: { locations: OfficeLocation[] }) {
  ...
  // Coverage corridors radiate from the HQ (Pune) to key markets — a subtle
  // "consulting coverage" motif, drawn faintly behind the markers.
  const hq = india.find((l) => l.kind === "office") ?? null;
  const coverageLinks = hq
    ? (
        ["mumbai", "ahmedabad", "nagpur", "hyderabad", "chennai", "bangalore", "delhi-ncr"]
          .map((s) => india.find((l) => l.slug === s))
          .filter((l): l is OfficeLocation => Boolean(l))
      )
    : [];
    ...
    <div>
      ...
      <div className="grid grid-cols-1 items-start gap-10 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        ...
        <div className="lg:sticky lg:top-24">
          <div className="flex flex-col items-center rounded-2xl border border-line/70 bg-gradient-to-b from-white to-[#F1F2EE] p-5 shadow-[0_16px_48px_-24px_rgba(20,64,58,0.28)] sm:p-7">
            <svg
              viewBox="0 0 100 101"
              aria-hidden="true"
            ...
            >
              ...
              <g clipPath="url(#reach-clip)" stroke="#14403A" strokeOpacity="0.06" strokeWidth="0.4" fill="none">
                <path d="M6 38 C30 32 62 34 96 30" />
                <path d="M8 60 C32 54 58 58 88 52" />
                <path d="M14 82 C30 78 44 80 60 76" />
              </g>

              {/* Coverage corridors from HQ — subtle, consulting-coverage feel */}
              {hq
                ? coverageLinks.map((t) => (
                    <line
                      key={`link-${t.slug}`}
              ...
              {india.map((loc) => {
                ...
                // HQ carries a persistent aura + larger marker; others light up when active.
                const showAura = isActive || isHQ;
                const baseR = isHQ ? 2.7 : 1.9;
                const markerR = isActive ? baseR + 0.6 : baseR;
                return (
            ...
            <div className="mt-6 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-xs text-muted">
              ...
              <span className="flex items-center gap-2">
                <span className="inline-block h-0 w-5 border-t border-jewel/40" />
                Coverage corridor
              </span>
            </div>
          </div>
